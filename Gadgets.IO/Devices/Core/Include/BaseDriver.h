...
namespace GadgetsIO
{
	...
	namespace Devices
	{
		...
		protected:
			...
			void AssignCallback(std::shared_ptr<Core::IAsyncCallback<Args...>> pCallback, std::shared_ptr<Core::IAsyncCallback<Args...>>& pTarget)
			{
				pTarget = pCallback;
			}

			template <>
			void AssignCallback(std::shared_ptr<Core::IAsyncCallback<>> pCallback, std::shared_ptr<Core::IAsyncCallback<>>& pTarget)
			{
				pTarget = pCallback;
			}

			template <typename... Args>
			void ExecuteCallback(std::shared_ptr<Core::IAsyncCallback<Args...>>& pTarget, Args... args)
			{
